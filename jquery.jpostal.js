...
function JpostalDatabase () {
	...
	this.find = function ( i_postcode ) {
		...
		for ( i = 0; i < this.address.length; i += 1 ) {
	...
	this.getStatus = function ( i_postcode ) {
		...
		if ( typeof this.map[postcode] === 'undefined' ) {
			...
			t_ms = (new Date()).getTime() - this.map[postcode].time;
			if ( t_ms < 5000 ) {
				// # 3
				st = 'waiting';
...
function Jpostal ( i_JposDb ) {
	...
	this.displayAddress = function () {
		...
		for ( key in this.options.address ) {
			s = this.formatAddress( this.options.address[key], this.address );
	...
	this.setSelectTagForPrefecture = function ( i_key, i_value ) {
		...
		for ( i = 0; i < el.options.length; i += 1 ) {
			p = el.options[i].text.indexOf( i_value );
			if ( 0 <= p ) {
